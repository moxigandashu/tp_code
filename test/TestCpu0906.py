...
devices='GQ4PONWSVKIVCESS'
pkg_name='com.cootek.smartdialer'
totaltime=10  #总时间，单位秒
...
def cpu_rate(pid, cpukel, devices,totaltime):
    ...
    for num in range(0, totaltime):
        ...
        listcpu = str(ts) + ' ' + devices + ' ' + cpu
        # cpu_mean = str(ts) + ' ' + cpu_mean
        result0 = "./list_cpu0907.txt"
        f = open(result0, 'a')
        f.write(listcpu + '\n')
        f.close()
        list_cpu.append(cpu)
...
if __name__ == "__main__":
    ...
    ts = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
    cpu_mean = str(ts) + ' ' + pkg_name + ' ' + cpu_mean
    # cpu_mean = str(ts) + ' ' + cpu_mean
    result2 = "./cpu_mean0907.txt"
    f = open(result2, 'a')
    f.write(cpu_mean + '\n')
    f.close()
